...
CHART_H_METRICS   = 230 if not COMPACT else 170
CHART_H_MATERIALS = 230 if not COMPACT else 170
CHART_H_SCENARIO  = 230 if not COMPACT else 170

CHART_W_RIGHT     = 900 if not COMPACT else 480
...
class CircularControl(tk.Canvas):
    ...
    def _draw_control(self):
        ...
        cx = self.width // 2
        cy = (self.height - 26) // 2  # Adjust for label space -20
    ...
    def update_chart(self, categories, baseline_values, current_values, colors=None, units=None):
        ...
        for i, category in enumerate(categories):
            ...
            self.create_text(
                ...
                text=f"{baseline_values[i]:.2f}", anchor="s",
            ...
            self.create_text(
                ...
                text=f"{current_values[i]:.2f}", anchor="s",
        ...
        self.create_text(
            ...
            text=f"{ymax:.2f}", anchor="e", tags="value",
...
class RecordBarChart(FuturisticChart):
    ...
    def update_chart(self):
        ...
        for i, record in enumerate(self.records):
            ...
            for j, key in enumerate(metrics):
                value = record[key]
                bar_height = (value / (max_value * 1.15)) * self.chart_height
        ...
        self.create_text(
            self.margin_left - 5, self.margin_top,
            text=f"{max_value:.2f}", anchor="e", tags="value",
            fill=COLORS["text_secondary"], font=("Segoe UI", 8)
        )
        self.create_text(
            self.margin_left - 5, self.height - self.margin_bottom,
    ...
    def draw_legend(self):
        ...
        items = [
            ...
            ("CO2 (kg)", "co2"),
            ("Cost (EUR)", "cost"),
            ("Brass (kg)", "virgin_brass_kg"),
            ("Plastic (kg)", "virgin_plastic_kg"),
        ]
...
"""
    ...
    def update_energy_mix(self, *args):
        ...
        if abs(total_display-100) > 0.1:
            self.energy_sum_label.config(text=f"Gesamt: {total_display:.0f}% (Bitte auf 100 % einstellen)", foreground=COLORS["negative"])
        else:
