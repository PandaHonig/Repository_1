...
const float    NOISE_MAX   = 30.0f;     // Keep at 30 (not 12).
const float    NOISE_CLIP_IN = 20.0f;   // Noise clip.
const int      MIN_DELTA_UP  = 20;      // Absolute min threshold.
const int      MIN_DELTA_DN  = 60;      // Lichtstärkeanpassung                 
const float    K_NOISE       = 7.5f;    // Noise multiplier.
const float    REL_FRAC      = 0.12f;   // Relative threshold.
...
float baseline   = 0.0f;
float raw_ema    = 0.0f;
float noise_ema  = 5.0f;  // Larger initial noise estimate.
char  state      = AMBIENT;
unsigned long tSolar = 0, tSolarPrint = 0;

// Restore baseline update control.
...
// Energiemix from Python (0.0 ~ 1.0).
...
// Add smooth read helper (from Solar_2.ino).
...
bool parseEmixLine(const String& line, float& solar, float& wind) {
  ...
  // Allow optional space after comma: ", wind=..."
  sSolar.trim();
  sWind.trim();
...
// Read 5 POTS and print one line: POTS: v1,v2,v3,v4,v5
...
void setup() {
  ...
  // -- Solar calibration (Solar_2.ino method) --
  curWin = W_SOLAR;
  tWinStart = millis();

  {
    warmup(PIN_SOLAR, DEFAULT);       // 5V reference warmup.
    unsigned long t0 = millis();
    long acc=0, cnt=0;
    while(millis() - t0 < SOLAR_CAL_MS){
      acc += readSmoothAnalog(PIN_SOLAR, 8);  // Use smooth read.
      cnt++;
      delay(10);
    }
    baseline = cnt ? (float)acc/cnt : 0.0f;
    raw_ema  = baseline;
    noise_ema= 5.0f;  // Larger initial noise estimate.
    state    = AMBIENT;
    baseline_update_counter = 0;  // Initialize counter.
...
void loop() {
  ...
  // -- Periodically read 5 POTS and send to serial --
  ...
  if(curWin == W_SOLAR){
    if(now - tSolar >= SOLAR_SAMPLE_MS){
      ...
      int   raw_val = readSmoothAnalog(PIN_SOLAR, 8);  // Use smooth read.
      float raw = (float)raw_val;
      float diff = raw - baseline;

      // Update noise only in AMBIENT (from Solar_2.ino).
      if (state == AMBIENT) {
        float nd = fabs(diff);
        if (nd > NOISE_CLIP_IN) nd = NOISE_CLIP_IN;  // Noise clip.
        noise_ema = (1.0f - NOISE_ALPHA) * noise_ema + NOISE_ALPHA * nd;
        if (noise_ema > NOISE_MAX) noise_ema = NOISE_MAX;
      }

      // Full threshold calculation (from Solar_2.ino).
      ...
      // State machine logic (from Solar_2.ino).
      char newState = state;
      switch (state) {
        case AMBIENT:
        ...
        case LAMP:
          if (diff < up_th - HYST)  newState = AMBIENT;  // Use hysteresis.
          break;
         
        case BLOCKED:
          if (diff > -(dn_th - HYST)) newState = AMBIENT;  // Use hysteresis.
          break;
      }

      // Noise handling on state change (from Solar_2.ino).
      if ((state == LAMP || state == BLOCKED) && newState == AMBIENT) {
        noise_ema = max(2.0f, noise_ema * 0.7f);  // Not too aggressive.
      }
